// bench_v02 — benchmark the v02 GOTO-style PoT kernel against OpenBLAS sgemm.
...
int main(int argc, char** argv) {
    CLI::App app{"bench_v02 — PoT v02 (GOTO-style, packed) vs OpenBLAS sgemm"};
    ...
    if (!skip_verify) {
        ...
        std::printf("  dynamic: ");
        if (!verify(mm_avx512_f32_1_u8_1_v02, M, N, K)) return 1;

        if (packed) {
            // verify pre-packed path
            PotTestData d(M, N, K);
    ...
    if (!packed) {
        double t_v02 = bench_kernel(mm_avx512_f32_1_u8_1_v02, M, N, K, repeats);
        print_row(M, N, K, t_blas, t_v02);
    } else {
        // Pre-pack B once, time only the kernel
        PotTestData d(M, N, K);
        uint8_t* Bp = aligned_new<uint8_t>((size_t)N * K);
        pot_pack_b_v02(d.B_u8, Bp, N, K);
